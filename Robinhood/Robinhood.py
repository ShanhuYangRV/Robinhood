...
class Robinhood:
	...
	# shanhu add: buy sell history to check how much I make
	buy_history = {}
	sell_hsitory = {}

	# shanhu note: this is only needed for bitcoin trading
	# and for now we are only using quotes and orders
	ENDPOINTS = {
	...
	def quote_data(self, stock=''):
		...
		if isinstance(stock, dict):
			...
			if stock == 'BTCUSD':  # shanhu edit: special case for bitcoin
				symbol = Robinhood.PAIRS[stock]
				assert symbol, 'unknown pair {}'.format(stock)
	...
	def quotes_data(self, stocks):
		...
		if 'BTCUSD' in stocks:
			stocks.remove('BTCUSD')
			btc_quote = [self.quote_data('BTCUSD')]
		else:
			btc_quote = []
	...
	def submit_sell_order(self,
		...
		if (trigger == 'stop'):
			...
			# shanhu edit: this contradicts with other logic: price should be none for stop loss sell order
			# if (price <= 0):
	...
	# }
	def trade_bitcoin(self, pair, **kwargs):
		...
		payload = {
			**{
				'account_id': self.get_account()['rhs_account_number'],
		...
		# Shanhu: need to set content type to json in header to upload the payload
		header_backup = self.session.headers
		self.session.headers['Content-Type'] = 'application/json'
		try:
			res = self.session.request('post', Robinhood.ENDPOINTS['orders'], json=payload, timeout=15)
			res.raise_for_status()
			self.session.headers = header_backup

			return res

			# ---- org code
			# res = self.session_request(Robinhood.ENDPOINTS['orders'], json_payload=payload, method='post',
			#                            timeout=5)
	...
	def order_status_bitcoin(self, order_id):
